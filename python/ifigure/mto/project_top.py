...
#         traceback.print_stack()
     ...
     def write_log(self, *args):
         ...
         txt1 = '::'.join([datetime.datetime.now().strftime("%Y_%m_%d_%H_%M"), threading.current_thread().name]) + '\n'
         fid.write(txt1)
     ...
     def tree_viewer_menu(self):
         ...
         if has_hg:
             menu.append(('Check Package Updates', self.onCheckPackageUpdate, None))
     ...
     def reorder_top_level(self):
         d = {'book':[],
              ...
              'others':[] }
         for name, child in self.get_children():
              if isinstance(child, PySetting): d['setting'].append(child)
              elif isinstance(child, FigBook): d['book'].append(child)
              elif isinstance(child, PyModel): d['model'].append(child)
              elif isinstance(child, PySol): d['sol'].append(child)
              elif isinstance(child, PySolver): d['solver'].append(child)
              else: d['others'].append(child)
         self._d = d['setting']+d['model']+d['solver']+d['sol']+d['book']+d['others']
