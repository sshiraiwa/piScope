...
class FigPage(FigObj):
    ...
    def get_axes(self, iax=None):
        ...
        c = 0
        if iax is None: return [x for x in self.get_children()
                                if isinstance(x[1], FigAxes)]

        for name, child in self.get_children():
            if isinstance(child, FigAxes): 
               if c == iax: return child
               c = c + 1
        for child in self.walk_tree():
            if isinstance(child, FigAxes): 
               if child._floating and child._generic_axes:
    ...
    def num_axes(self, include_floating = False):
        c = 0
        for name, child in self.get_children():
            if isinstance(child, FigAxes): c = c + 1
        if not include_floating: return c
        for child in self.walk_tree():
            if isinstance(child, FigAxes): 
               if child._floating and child._generic_axes:
                   c = c + 1
    ...
    def walk_axes(self):
        c = 0
        for name, child in self.get_children():
            if isinstance(child, FigAxes): yield child
    ...
    def set_bmp_update(self, value):
        ...
        if not value: self.reset_axesbmp_update()
    ...
    def set_section(self, *args, **kargs):
        ...
        if (isinstance(args[0], list) or
            ...
            if len(args)>0: nrow=args[0]
            if len(args)>1: ncol=args[1]
            if len(args)>2: asym=args[2:]
            nrow = [nrow]*ncol
        ...
        for m in mareas0: areas.append(m)
...
#       e.Skip()
    ...
    def get_nipage(self):
        ...
        for name, child in self.get_parent().get_children():
           try:
              if child is self: ipage=npage
              if child.isFigPage(): npage=npage+1
           except Exception:
              pass
        return npage, ipage
    ...
    def onZoom(self, evt, figaxes):
        for ref in self._zoom_cb:
           if ref() is not None: ref().onZoom(evt, figaxes)
    def onPan(self, evt, figaxes):
        for ref in self._pan_cb:
           if ref() is not None: ref().onPan(evt, figaxes)
...
#        self._resize_cb = [x for x in self._resize_cb if x() is not figobj]
    ...
    def set_suptitle(self, txt, a=None, size=None, color=None):
        info = self.getp('suptitle_labelinfo')
        info[0] = txt
        if size is not None: info[-1] = size
        if color is not None: info[1] = color
        if self._title_artist is not None:
            self._title_artist.set_text(txt)
            if info[-1] != 'default': self._title_artist.set_size(info[-1])
            if info[1] != 'default': self._title_artist.set_color(info[1])            
...
#        print "saving fig_page data"
    ...
    def convert_to_tex_style_text(self, mode=True):
        ...
        if self._title_artist is None: return
        if mode:
