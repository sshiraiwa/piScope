...
#            logging.exception("Module Method Call Failed")
    ...
    def load_module(self, file):
      try:
        ...
        if allow_only_one_active:
            if not file in active_module: active_module[file] = []
    ...
    def activate(self):
        ...
        if file in active_module:
            # cleaning list first
            active_module[file] = [m for m in active_module[file] if m() is not None]
...
#        print "loading", self.getvar("module file")
    ...
    def rename(self, new,  ignore_name_readonly = False):
        ...
        oname=self.name
        oowndir = self.owndir()
        super(PyModule, self).rename(new, ignore_name_readonly = 
