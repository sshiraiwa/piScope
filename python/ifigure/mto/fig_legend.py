...
class FigLegend(FigObjGPHolder):
    ...
    def property_in_palette(self):
        return  ["position", "labels", "box"],[
                ...
                ["facecolor_2", "edgecolor_2","linewidthz", "plinestyle_2","alpha_2", "legendshadow"],
                ]
    ...
    def make_newartist(self):
        ...
        if self.getp('legendhandle') is not None:
            ...
            if len(v) == 0: self.delp('legendhandle')
            else:self.setp('legendhandle', v)
        ...
        if len(artistlist) != 0:
            ...
            for a in artistlist:
                if self.get_figaxes() is None:
                    if a in c.legends: c.legends.remove(a)
                else:
                    if a in c.artists: c.artists.remove(a)
...
#        return FigGrp.get_artist_extent(self, a)
    ...
    def drag_a(self, a, evt, shift = None, scale=None):
        redraw, scale=super(FigLegend, self).drag_a(a, evt, shift=shift, scale=scale)
        return redraw, scale
...
#        text = self._get_texts()[0]
    ...
    def double_click_on_canvas(self, event, canvas):
        import matplotlib
        if not isinstance(self._hit_a, matplotlib.text.Text): return
...
#        print idx
    ...
    def onRefreshLegend(self, evt):
        ...
        ax = self.get_figaxes()        
        if ax is not None: ax.set_bmp_update(False)
        canvas.draw()
