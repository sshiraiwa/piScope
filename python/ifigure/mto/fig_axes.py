...
#        self.child=[]
        ...
        self._edge_only = [False,False,False,False] #top, bottom, left, right
...
#       fig_axes store all getp() without this modification
    ...
    def can_have_child(self, child = None):
        ...
        if isinstance(child, FigPage): return False
        if isinstance(child, FigBook): return False
        if (isinstance(child, FigAxes) and 
            not isinstance(child, FigInsetAxes)): return False
        return True
...
#                                  +self._zaxis + self._caxis)]
        ...
        for i in self._xaxis:
            ...
            item.append(["axlabel", "axrangeparam13",
                         ...
                         "axlotsize", 
                         "axtlcolor", "axticks"])
            if not self._3D:item[-1].append("axxpos")            
        for i in self._yaxis:
            ...
            item.append(["axlabel", "axrangeparam13",
                         ...
                         "axlotsize", 
                         "axtlcolor", "axticks"])
            if not self._3D:item[-1].append("axypos")
        if self._3D:
...
#                            "axlsize", 
        ...
        for i in self._caxis:
            tab.append(i.name)
            item.append(["axlabel", "axrangeparam13o", "cmap3", "cxclipunder", "cxclipover"])
...
#           self.backup_margin_param()
           ...
           if not "x" in params: self._xaxis[-1].add_ax_idx(ppp)
           if not "y" in params: self._yaxis[-1].add_ax_idx(ppp)
           if self._3D:
               if not "z" in params: self._zaxis[-1].add_ax_idx(ppp)               
...
#               p = self.get_axis_param(name)
           ...
           if self._hastwin[0]:self.twinx(new_axesparam=False)
           elif self._hastwin[1]:self.twiny(new_axesparam=False)  
           ...
           for idx, a in enumerate(self._artists):
               ...
               for key in params:
                   ...
                   if not self._3D:  params[key].set_tickparam(a, self)
...
#            data1, data2 = target.figobj.get_slice(x, y)
    ...
    def _update_cursor_line(self, idx, x, y):
        if idx == 0:
            if len(self._cursor1) < 2: return
            self._cursor1[0].set_xdata([x,x])
            self._cursor1[1].set_ydata([y,y])
        else:
            if len(self._cursor2) < 2: return
    ...
    def valid_cursor(self):
        val = []
        if len(self._artists) == 0: return val
...
#        traceback.print_stack()
        ...
        rect = [area[0]+tm[0]*area[2], 
                ...
                (1-tm[0]-tm[1])*area[2], 
                (1-tm[2]-tm[3])*area[3]]
        if self._nomargin_mode: ignore_pagemargin = True      
...
#           range0= max((1e-16, range[0]))
           range1 = range[1]
           if range[1] < range0: range1 = 2*range0
...
#        print 'send range change event'
    ...
    def set_title(self, value, a=None):
        ...
        if isinstance(value, str):
            ...
            v2 = [x for x in value]
        if str(value[2])=='default': v2[2] = tfont
        if str(value[3])=='default': v2[3] = tweight
        if str(value[4])=='default': v2[4] = tstyle
        if str(value[5])=='default': v2[5] = tsize
...
#       self.onForceUpdate(e)
    ...
    def onAddInsetAxesCanvas(self, evt):
        ...
        child = self.get_child(ichild)
        child.realize() 
        ifigure.events.SendChangedEvent(self.get_figpage(), evt.GetEventObject())
...
#        print "loading  fig_axes data"        
        ...
        if 'FigAxes' in data:
            ...
            if 'title_labelinfo' in val:
                if isinstance(val['title_labelinfo'], str):
                    dprint1('stored title_lableinfo is string!!!!. Executing recovery section')
    ...
    def cla(self):
        ...
        for name, child in self.get_children():
            ...
            if isinstance(child, FigText):continue
            child.destroy()
    ...
    def twinx(self, new_axesparam=True):
        ...
        if new_axesparam: 
            ...
            for p in self._yaxis: p.tick_both = False
#           self._axis_name_table[p.name] = len(self._artists)-1            
    ...
    def twiny(self, new_axesparam=True):
        ...
        if new_axesparam: 
            ...
            for p in self._xaxis: p.tick_both = False            
#           self._axis_name_table[p.name] = len(self._artists)-1            
...
#        self._artists[0].set_axis_bgcolor(value)
    ...
    def _set_3d_pane_color(self, code):
        ...
        if len(self._artists) == 0: return
        if not isinstance(self._artists[0], Axes3DMod): return
        a = self._artists[0]
    ...
    def convert_to_tex_style_text(self, mode=True):
        ...
        for a in self._artists:
            ...
            if self._3D:labels.append(a.zaxis.label)
        if mode:
...
#        print self._parent._artists[0].get_window_extent().get_points()
    ...
    def calc_rect(self, ignore_pagemargin = False):
        ...
        rect = self.convert_transform(self.getp("inset_transform"), 'figure', rect)
#        print 'rect', rect
        self._rect = rect
...
#
    ...
    def dragdone_a(self, a, evt, shift=None, scale=None):
        ...
        h = [UndoRedoFigobjMethod(a, 'inset_rect', rect) for a in self._artists]
...
#        hist = self.get_root_parent().app.history
    ...
    def hndl_dclick(self, canvas):
        ...
        requests = {self._parent: self._parent.compute_new_range(request = request)}
        requests = canvas.expand_requests(requests)
