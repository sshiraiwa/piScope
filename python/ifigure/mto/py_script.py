...
class PyScriptLink(TreeLink, AnsHolder):
    ...
    def onRunQuick(self, e):
        if not self.is_linkalive: return 
        t = self.get_linkobj()
...
#        self.onRunQuick(e)
    ...
    def _is_in_pysol(self):
        ...
        p = self
        while p is not None:
            if isinstance(p, PySol): return True
            p = p.get_parent()
        return False
...
#        path =  [model.name]+ model.get_relative_path(self)
    ...
    def onRunAtSol(self, e, title = 'Run at sol'):
        ...
        app = wx.GetApp().TopWindow
        app.proj_tree_viewer.OpenPanel(list, self, 'handle_run_at_sol', title = title)
...
#        print file
    ...
    def rename(self, new,  ignore_name_readonly = False):
        ...
        super(PyScript, self).rename(new, ignore_name_readonly = 
