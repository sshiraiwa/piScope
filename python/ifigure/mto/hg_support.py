...
#                                    traceback=traceback.format_exc())       
   ...
   def hg_add_no_binary(repo, dir, exclude = None, include = None):
       ...
       if include is None: include = ['.tree_data_hg']
...
#           print item
   ...
   def _add_include_exclude(obj):
       if not obj.hasvar('include'): obj.setvar('include', ['.tree_data_hg'])
       if not obj.hasvar('exclude'): obj.setvar('exclude', [])
...
#               ["user", os.getenv("USER"), 0, None]]
           ...
           no_treedata = True if '.tree_data_hg' in self.getvar('exclude') else False
           self.do_commit(str(value[1][0]), user=str(value[1][1]), no_treedata = no_treedata)
...
#           else:
           try:
               self._hg_rev_str = '(rev.'+str(repo.hg_rev())+'/'+str(max(l)) +flag+')'
           except:
               import traceback
               traceback.print_exc()
...
#                  print txt
       ...
       def add_hg_menu(self, menu):
           if self.get_extfolderpath() is not None: return menu          
           if has_hg:
               if has_repo(self):
