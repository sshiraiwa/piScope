...
#        self._container=None
        ...
        self.setp("args", args)
        if len(args)  != 0:
           logging.warning("FigObj:len(args) != 0: Not all arguments may not have processed")
           print(self)
           print(args)
...
#        return ['zorder', 'use_var', 'rasterized']
    ...
    def var2p(self, list):
        ...
        a = {}
        for name in list:
           if not self.hasvar(name):continue
           v = self.eval(name)
           self.setp(name, v)
           a[name] = v
        return a
    ...
    def get_container(self, *args, **kywds):
        ...
        if ax is None or ax is self:
           ...
           if len(page._artists) == 0: return None
           return page._artists[0]   
        else:
           if len(ax._artists) == 0: return None
...
#               traceback.print_stack()
    ...
    def set_artist(self, alist0):
        if isiterable(alist0): alist = alist0
        else: alist = [alist0]
        self._artists = alist
    ...
    def walk_figobj(self):
        for obj in self.walk_tree():
            if isinstance(obj, FigObj): yield obj

    def set_zorder(self, z1, a = None):
        self.setp('zorder', z1)
        for a in self._artists:
            a.set_zorder(z1)
        if len(self._artists) != 0: self.set_bmp_update(False)
    ...
    def swap_zorder(self, z1, z2, get_action = False):
        ...
        for figobj in objs:
            if get_action:
                h.append(UndoRedoFigobjMethod(figobj._artists[0], 'zorder', z1))
            else:
    ...
    def set_zorder_front(self, get_action=False):
        ...
        if self.get_zorder() <= max(l1):
            ...
            if l1.count(self.get_zorder()) == 1 and self.get_zorder()==max(l1): return ret
            if get_action:
    ...
    def set_rasterized(self, value=None, a=None):
        ...
        if value is not None: self.setp('rasterized', value)
...
#        self.set_rasterized(True)
    def set_container_idx(self, value, a=None):
        ...
        if isinstance(self, XUser): self.unset_ax()
        if isinstance(self, YUser): self.unset_ay()
        if isinstance(self, ZUser): self.unset_az()
...
#           ifigure.server.Server().export_data(data) 
    ...
    def export(self,  a = None):
        if a is None:
            ...
            if not a in self._artists:return []
            sel = [a]
            
        return [self.get_export_val(a) for a in sel]

    def get_export_val(self, a):
        raise NotImplementedError('get_export_val is missing in '+str(self.__class__))
    ...
    def add_child(self, *args, **kargs):
        ...
        if not 'z_base' in kargs:
           if figpage is not None:
              z = max([obj2.getp('zorder') for obj2 in figpage.walk_figobj()])
           else:
              z = 1
        else:
           z_base = kargs['z_base']
...
#        self.get_child(idx=idx).set_bmp_update(False)
        ...
        if kargs['keep_zorder']: return idx
...
#        for obj in self.get_child(idx).walk_figobj():
        ...
        if 'z_base' in kargs: z_base._z_base = z
        return idx
#
...
#     is taken at once
    ...
    def getp(self, name=None):
       if name is None: return self._attr
...
#                 ('!',   None, None),
    ...
    def get_root_figgrp(self):
        ...
        if not in_group: return None
        while p is not None:       
           if not isinstance(p.get_parent(), FigGrp): break
           p = p.get_parent()
        return p
            ...
            if self._picker_a_type == 'area':
               ...
               if shift:
                  if abs(dx) < abs(dy) : 
                   ...
                   dy = 0
               rec[0]=rec[0]+dx; rec[1]=rec[1]+dx
               rec[2]=rec[2]+dy; rec[3]=rec[3]+dy

            else:
               if ((loc & 1) != 0): rec[0] = evt.x
               if ((loc & 2) != 0): rec[1] = evt.x
               if ((loc & 4) != 0): rec[2] = evt.y
               if ((loc & 8) != 0): rec[3] = evt.y
...
###   group/ungroup (ungroup is defined in FigGrp)
    def group(self, member):
        ...
        p = self.get_parent()
        if p is not None:
            p.add_child(p.get_next_name(gp.get_namebase()), gp, keep_zorder=True)
        
        if not self in member: member=[self]+member
        for obj in member:obj.move(gp, keep_zorder=True)
        return gp
...
#        pickle.dump(val, fid)
    ...
    def store_loaded_property(self): 
        if self.hasp("loaded_property"): return False
...
#        ps = properties_in_file_1(self)
        ...
        if is_new_stored_prop: self.delp('loaded_property')
        return data
...
#        print(loaded_prop)
        if "format" in val:
           if val["format"]==2:
              ...
              for k in attr:
                 self.setp(k, attr[k])
        if not self.hasvar("kywds"): self.setvar("kywds", {})
    ...
    def _update_range(self, range, de):
        ...
        if not isinstance(de[0], masked):
            if range[0] is None:
                ...
                if de[0] is not None: range[0] = min([range[0], de[0]])
        if not isinstance(de[1], masked):
            if range[1] is None:
                range[1] = de[1]
            else:
                if de[1] is not None: range[1] = max([range[1], de[1]])
        return range
#
#
#
