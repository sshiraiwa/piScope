...
class IDLfile(Namelist):
   ...
   def __setitem__(self, key, value):
       ...
       if isinstance(value, collections.OrderedDict):
          ...
          value2=value
       if hasattr(value, '_var0'): value2._var0 = value._var0
       super(PyContents, self).__setitem__(key, value2)
...
class MDSPlusTreeVVV(NamelistVVV):
   def get_drag_text2(self, key):
       ...
       text for dnd from var viewer
       '''
       text=self._td.get_full_path()+self._path+'.mds_eval("'+key+'",'+self._td.get_full_path()+')'
       return text
...
#          value2=MDSPlusTree(self._td(), value)
   ...
   def get_varviewer_value(self, td, keylist):
       ...
       txt = '","'.join(keylist)
       if not isinstance(td, weakref.ProxyType): td = weakref.proxy(td)
       val._td = td
...
#       td = self._td()
       ...
       tree=td.getvar("tree")
       if shot is None: shot=td.eval("shot")
