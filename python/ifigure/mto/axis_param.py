...
class AxisVisualParam(object):
    ...
    def _set_label(self, txt): 
        ...
        if self.labelinfo[2] != 'default': txt.set_family(self.labelinfo[2])
        if self.labelinfo[3] != 'default': txt.set_weight(self.labelinfo[3])
        if self.labelinfo[4] != 'default': txt.set_style(self.labelinfo[4])
...
class AxisZParam(AxisParam):
    ...
    def set_artist_rangeparam(self, a):
        if not hasattr(a, 'set_zlim'): return
...
#           ifigure.events.SendPVAddFigobj(figaxes)
    ...
    def has_cbar(self):
        if  self._cb is None: return False
        if self._cb() is None: return False
        return True
    def cbar_shown(self):
