...
#                print self.get_figpage()._artists
    ...
    def set_gp_figaxes(self, p, figaxes, no_conversion = False):
        ...
        if len(figaxes._artists) == 0: return
        if no_conversion: return
...
#        t_from = gp.get_gp_transform(dir='both')
    ...
    def set_gp_point(self, i, x, y, trans=None):
        if trans is not None: self._gp_points[i].trans = trans
...
#        traceback.print_stack()
    ...
    def load_data2(self, data):
        if not 'GenericPointsHolder' in data: return
...
class GenericPoint(object):
    ...
    def dict_to_gp(self, d, holder):
        ...
        if d["ax"] is None:
            ...
            area = [abs((xy[1][2]-xy[1][0])*(xy[1][3]-xy[1][1])) for xy in rect]
...
#                    self.y=d["y"]
#                    break
            if hasattr(self, '_d_bk'):  del self._d_bk
        return self
