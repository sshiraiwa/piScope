...
class TreeDict(object, metaclass=BaseAttr):
    ...
    def compact_name(self):
        ...
        if len(txt) < 32: return txt
        else:
            names = [x for x in reversed([t._name for t in self.walk_tree_up()])]
            for x in range(len(names)-2):
               names[x+1] = '.'
               if len('.'.join(names)) < 32: return '.'.join(names)
    ...
    def add_child(self, *args, **kargs):
        ...
        if obj is not None:
           ...
           while hasattr(self, bname1): 
               conflict = True
               warning.append(self.get_full_path() +' already has attr '+ bname1)
               bname1 = bname1 + str(i)
               i = i+1
...
#        return self._do_get_full_path('')
    ...
    def walk_tree(self, stop_at_ext = False):
       ...
       yield self
       if stop_at_ext and isinstance(self, ExtMixIn): return
#       child=self.get_first_child()
       if len(self._d) == 0: return 
       child = self._d[0]
       k = 0
...
#           child=self.get_next_child(child)
    ...
    def find_common_parent(self, td_list):
        ...
        while p is not None:
           flag = True 
           for t in td_list:
               if t is None: return self.get_root_parent()
               if p is t: break
...
#       print self, args
    ...
    def get_pyfolder(self):
        ...
        p = self
        while p is not None:
           if isinstance(p, PyFolder): return p
           p = p.get_parent()
        return None
    ...
    def get_pymodel(self):
        ...
        p = self
        while p is not None:
           if isinstance(p, PyModel): return p
           p = p.get_parent()
        return None
    ...
    def get_figbook(self):
        ...
        p = self
        while p is not None:
           if isinstance(p, FigBook): return p
           p = p.get_parent()
        return None
    ...
    def get_figpage(self):
        ...
        p = self
        while p is not None:
           if isinstance(p, FigPage): return p
           p = p.get_parent()
        return None
    ...
    def get_figaxes(self):
        ...
        p = self
        while p is not None:
           if isinstance(p, FigAxes): return p
           p = p.get_parent()
        return None
    ...
    def get_variables(self):
        ...
        p = self
        while p is not None:
           if isinstance(p, AbsNamespacePointer):  return p
           p = p.get_parent()
        return None
    ...
    def _eval(self, name='', use_np=False):
       try:
          ...
          lc[root.name] = root
          m = self.get_pymodel()
          if m is not None: lc['model'] = m
          if m is not None: lc['param'] = m.param
          m = self.get_figpage()
          if m is not None: lc['page'] = m
          m = self.get_figbook()
          if m is not None: lc['book'] = m
...
#          pass
    ...
    def getvar(self, *args):
       if len(args) == 0: return self._var
       try:
           if len(args) == 1: 
    ...
    def getnote(self, name=None):
       if name is None: return self._note
       try:
    ...
    def get_contents(self, *args, **kargs):
        ...
        if not isinstance(self._var0, PyContents): return None
        contents = self._var0
        for key in args:
           contents = contents[key]
        if hasattr(contents, 'eval'): return contents.eval(self, **kargs)
        return contents
    ...
    def tree_viewer_menu(self):
       ...
       if not isinstance(self, ExtMixIn):       
           ...
           if self.can_have_child():
               ...
               if (self.hasvar('subtree_path') and
                   os.path.exists(self.getvar('subtree_path'))):
                       menu.append(('Export '+self._name, self.onSaveSubTree, None))
               else:
                   menu.append(('-Export '+self._name, None, None))
               menu.append(('Export '+self._name + ' As...', self.onSaveSubTreeAs, None))

       from ifigure.mto.py_code import PyFolder

       if self.can_have_child(PyFolder()):
           if isinstance(self, ExtMixIn):
               menu.append(('Reload File Tree...', self.onReloadExtTree, None))
               menu.append(('Change Dir. to here', self.onChangeDirToExt, None))       
            ...
            if p is not None:
               if hasattr(p, new_name):
                  ret = dialog.message(app,
                        'attribute name (' + new_name +') is already used',
                        'Name Conflict', 
                        0)
                  return None
               if new_name == 'tmp':
                  ret = dialog.message(app,
                        'attribute name (' + new_name +') is reserved',
                        'Reserved Name', 
                        0)
                  return None
            return new_name
...
#        if not self._can_have_child:return
        if not self._has_private_owndir: return
        if self.has_owndir():
           try:
...
#        if self._can_have_child:
    ...
    def write2shell(self, value, name):
        ...
        '''
        root = self.get_root_parent()
        if not hasattr(root, 'app'): return
        if root.app is None: return
...
#        root.app.shell.push('print %r' % txt, silent = True) 
    ...
    def save2(self, fid=None, olist=None):
        ...
        h2={"name": self.__class__.__name__,  
            ...
            "note":self._note,
            "format": 2}
        if self._save_var: h2["var"]= self._var
#        print h2
        if self._nosave_var0: h2["var0"] = None
...
#               obj._var0.set_td(obj)
        ...
        for i in range(0, h2["num_child"]):
            try:
                ...
                #print('File broken !!!!!')
                break
            if child is not None: obj.add_child(child, keep_zorder=keep_zorder)
...
#        print 'returning', obj
    ...
    def load_data2(self,data):
        if not "TreeDict" in data: return
        d = data["TreeDict"]
        if len(d) >1:
    ...
    def resolve_olist_nlist_map(self, oid, olist, nlist):
        ...
        if obj is not None: return weakref.proxy(obj)
        return None
    ...
    def duplicate(self, new_parent, new_name='', save_script_link=False,
        ...
        if new_name == '': new_name = self.name
#        sc = st_scratch + '_' + str(os.getpid()) + name
        ...
        if save_script_link:
            ...
            for s in scripts: s._save_link = True
        flag, tmpdir = self.save_subtree(sc, maketar=False)
        if save_script_link:
            for s in scripts: s._save_link = False
...
#         print olist, nlist
         ...
         if isinstance(td, ExtMixIn): tmp_obj = PyExtFolder()
         else: tmp_obj = PyFolder()
...
#         print time.time()-t0
         ...
         if names.count(oname) != 0:
             td.rename(self.get_next_name(oname+'_loaded'), ignore_name_readonly=True)

#         if not td._can_have_child and td.has_item(): self.mk_owndir()
         if not td._has_private_owndir and td.has_item(): self.mk_owndir()
...
#         print time.time()-t0
    ...
    def search(self, txt, dir = 0, include_self=True):
        ...
        for x in g():
            if x is self: 
                 start = True
                 if not include_self: continue
            if not start: continue
            if x._name.upper().find(txt.upper()) != -1: return x
        return None
...
#        d=os.path.join(base, dname)
        dname='.###ifigure'+'_'.join((os.path.splitdrive(str(path))[1]).split(os.sep))
