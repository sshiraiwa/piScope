            ...
            if self._drag_hit == -1:
                 ...
                 if self._hit_seg_i != -1: 
                    ...
                    if len(segpath[self._hit_seg_i])==2:
                        m =  m + [("Convert to curve",  self.onConvCurve, None),]
                    else:
...
#            hist = self.get_root_parent().app.history
 ...
 #       hist = self.get_root_parent().app.history
    ...
    def onMakeCorner(self, evt):
        ...
        p[self._drag_hit-1]=(p[self._drag_hit-1][0],p[self._drag_hit-1][1],2)
        p[self._drag_hit+1]=(p[self._drag_hit+1][0],p[self._drag_hit+1][1],2)
    ...
    def make_newartist0(self,  pin, draw_arrow = True):
        ...
        if self.getvar('arrow1')[0]:
            ...
            b = matplotlib.patches.FancyArrowPatch(posA=verts[0]+dx, 
                        ...
                        arrowstyle=self.getvar('arrow1')[1],
                        mutation_scale=1,)
            aaa.append(b)
        if self.getvar('arrow2')[0]:            
            ...
            c = matplotlib.patches.FancyArrowPatch(posA=verts[-1], 
                                               posB=verts[-1]-dx,
                        connectionstyle = 'arc3',
                        arrowstyle=self.getvar('arrow2')[1],
                        mutation_scale=1,)
            aaa.append(c)
        return aaa
...
#                            evt.x, evt.y)
    ...
    def drag_a(self, a, evt, shift = None, scale=None):
        redraw, scale=super(FigCurve, self).drag_a(a, evt, shift=shift, scale=scale)
        if self._drag_hit != -1:
