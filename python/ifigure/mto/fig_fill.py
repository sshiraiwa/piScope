        ...
        if val == True:
           for a in alist:
               if isinstance(a, Polygon3DGL):
                  ...
                  x=np.concatenate([p.vertices[:,0] for p in a.get_paths()])
                  y=np.concatenate([p.vertices[:,1] for p in a.get_paths()])
...
#            print('double check for picker')
            if hasattr(artist, 'get_verts'):
               ...
               if hit: return True, {'child_artist': artist}
            return False, {}
    ...
    def get_yrange(self, yrange=[None,None], xrange=[None, None], scale='linear'):
        ...
        if (xrange[0] is not None and
            ...
            if self._mpl_cmd == 'fill':
                ...
                if scale == 'log': ym[x < 0] = np.ma.masked
...
#
    def save_data2(self, data=None):
        ...
        if var["x2"] is not True: var["x2data"] = self.getp("x2")
        if var["y2"] is not True: var["y2data"] = self.getp("y2")
        if var["where"] is not True: var["wheredata"] = self.getp("where")
