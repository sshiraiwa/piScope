...
#      def start(self, c_type, w_type):
      ...
      def is_any_worker_alive(self):
          if self.pool is not None: return self.pool.is_any_alive()
          return False           
      @classmethod
...
#                       print 'title only job'
                       ...
                       CallAfter(viewer.postprocess_skipped_data, a.fig_mds(), a)
...
#                   print 'normal job'
               for key in ana_groups:
                   ...
                   if (a.ipage == -1): continue   ## global analsys has always its own group
                   if ((a.ipage == key[0] and  ## same shot same page same panel 
                       ...
                       (a.ishot == key[1] and  ## same shot runs by same worker
                        not flag[1]       and  ## if not parallelizing panels and pages
                        not flag[2])      or
                       (not flag[0]  and 
                        not flag[1]  and 
                        not flag[2])): 
                   ...
                   ana_groups[(a.ipage, a.ishot, a.isec, ig)] = AnaGroup(a.ipage==-1, [a])
...
#           print len(ana2)
           ...
           for k, ana_group in enumerate(ret):
               ...
               tmp = sorted([(a.ishot, a)  for a in ana_group], key=lambda x:x[0])
...
#               self._nextqueue.put('stop')
               ...
               if not task.book in self._waitinglist: continue
...
#                    print 'error message', 
   ...
   def request_gui_update(self, ana_group, idx):
       ...
       for k in idx:
           a = ana_group[k]
           if not hasattr(a, 'fig_mds'): continue
           fig_mds = a.fig_mds()
           if fig_mds is None: continue
...
class ScopeEngine(object):
    ...
    def close_engine(self):
        if not self._engine_open: return
        ScopeEngine.scope_count = ScopeEngine.scope_count - 1
            ...
            for name, child in ax.get_children():
                if isinstance(child, FigMds):
                     ...
                     for k, s in enumerate(shots):
                         ss, prevshot = child.eval_shotstr(s, prevshot, 
                                                           self._cur_shot)
                         if ss is None and prevshot is None: return None
                         if ss == 'm' or ss == 'n':
...
#       postprocessing after data is loaded from MDSplus
        ...
        for key in child._shot:
            if child._shot[key] == ana.ishot: 
                del child._shot[key]
                break
        if flag: child._shot[ana.shot] = ana.ishot
...
#        self.start_pool()
    ...
    def onImportDW(self, evt, file = ''):
        ...
        if not filename in RECENT_FILE: RECENT_FILE.append(filename)
...
#           ax.apply_nomargin_mode()
           ...
           if 'global_defaults' in ps[iax]:
               ...
               # set grid
               v = ax.get_grid(); v[0] = bit & 2**5 != 0; ax.set_grid(v)
               v = ax.get_grid(); v[1] = bit & 2**6 != 0; ax.set_grid(v)
...
#        self._handle_apply_abort()
        ...
        if not is_safename(name):
            while not is_safename(name):
               ret, name = dialog.textentry(self,
                   'Enter a new book name ( ' + name+ ' is not valid name)', 
    ...
    def onApply(self, evt):
        if not self.check_valid_worker_type(): return
        self.debug_mode = False
        self._handle_apply_abort()
        evt.Skip()
