...
#                    a.set_result(v != None, v)
                ana_group = xxx[1]()
                if ana_group is None: return None
...
#       self.results= mp.Queue() 
       ...
       for i in range(num):
           if type=='direct':
               w = MDSMPDirectWorker(self.tasks, self.results, kwargs['translater'])
           else:
               w = MDSMPProxyWorker(self.tasks, self.results, str(host), str(port))
           self.workers[i] = w
           time.sleep(0.1)
...
class MDSTHWorkerPool(MDSWorkerPoolBase):
   def __init__(self, num, *args, **kwargs):
       ...
       for i in range(num):
           if type=='direct':
               w = MDSTHDirectWorker(self.tasks, self.results, kwargs['translater'])
           else:
               w = MDSTHProxyWorker(self.tasks, self.results, str(host), str(port))
           self.workers[i] = w
           time.sleep(0.1)
       for w in self.workers: w.start()
