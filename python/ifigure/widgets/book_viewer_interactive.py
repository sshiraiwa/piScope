...
def allow_interactive_call(method):
    ...
    def method2(self, *args, **kargs):
       ...
       if update: 
           ...
           ifigure.events.SendPVDrawRequest(self.book, wait_idle=True, refresh_hl=False)
       return obj
...
class BookViewerInteractive(object):
    ...
    def update(self, value):
        ...
        if value:
           ...
           if f_page is not None:
               self._z_base = max([obj2.getp('zorder') for obj2 in f_page.walk_figobj()])
           else:
               self._z_base = 1
    ...
    def title(self, txt, size=None, color=None):
        ...
        if len(fig_ax._artists) == 0:
            ...
            v[0] = txt
            if size is not None: v[-1] = 'default' if size=='default' else float(size)
            if color is not None: v[1] = color
    ...
    def _set_label(self, txt, name, size, color):
        ...
        if txt is None and size is None: return v
        if txt is not None: v[0] = txt
        if size is not None: v[-1] = 'default' if size=='default' else float(size)
        if color is not None: v[1] = color
    ...
    def contour(self, *args, **kargs):
        ...
        if not "FillMode" in kargs : kargs["FillMode"] = False
        try:
