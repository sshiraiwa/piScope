...
class RangeRequestMaker(object):
    ...
    def make_range_request_pan(fig_ax, auto=True, requests = None):
        ...
        if fig_ax._3D: arr.extend(fig_ax._zaxis)
 
        if requests is None: requests = {}
        if not fig_ax in requests: requests[fig_ax] = []
    ...
    def make_range_request_zoom(fig_ax, direction, range, auto, ax, requests = None):
        ...
        if direction   == 'x': arr = fig_ax._xaxis
        elif direction == 'y': arr = fig_ax._yaxis
        elif direction == 'z': arr = fig_ax._zaxis
        if requests is None: requests = {}

        if not fig_ax in requests: requests[fig_ax] = []
        for x in arr:
            if not ax in fig_ax.get_axes_artist_by_name(x.name): continue
            value = x.get_rangeparam()
            value[1] = auto
            value[2] = range
            value[6] = False
    ...
    def make_samexy_request(fig_ax, direction, auto, mode=1, requests=None):
       ...
       attr = '_'+direction+'axis'
       if not hasattr(fig_ax, attr): return {}
       g = getattr(fig_ax, attr)
       ...
       for f_ax in f_page.walk_axes():
           ...
           if not hasattr(f_ax, attr): continue
           if not f_ax in requests: requests[f_ax] = []
    ...
    def _add_induced_range_request(requests):
        ...
        for key in irequests:
            if not key in requests:requests[key] = []
...
#            f.append((ifigure.events, 'SendRangeChangedEvent', (key,)))
        if len(a) == 0: return
        window = self.GetTopLevelParent()
        GlobalHistory().get_history(window).make_entry(a, finish_action=f, menu_name=menu_name)
...
#        self.set_axrangeparam0(request[-1][0])
    ...
    def adjust_axes_range(self, get_action=False):
        use_auto = True
        try:
           if self.get_figbook()._lock_scale: use_auto = False
        except:
           pass
        ...
        def en_int_sym(range, mode):
            if range[0] is not None:
               if np.iscomplex(range[0]):
                  range = sorted([float(np.real(range[0])), float(np.real(range[1]))])
               if mode[2]:
        ...
        for ax in self._xaxis:
           ...
           if request is not None:
              for name, value in request:
                  if name == ax.name: 
                      base, auto, range, scale, symloglin, symscale, mode = _value2param(value)
                      break
