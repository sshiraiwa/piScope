...
#        super(ifigure_canvas_draghandler_zoom, self).__init__()
    ...
    def get_artist(self):
        ...
        if isinstance(self.a, list): return self.a
        return [self.a]
...
class draghandler_base2(draghandler_base):
    def bind_mpl(self, event):
        if self.mpl_id is not None: self.unbind_mpl()
        self.st_event=event
...
class ifigure_canvas_draghandler_cursor(draghandler_base):
    ...
    def bind_mpl(self, event):
        if self.mpl_id is not None: self.unbind_mpl()
        if self.panel.toolbar.mode != 'cursor': return
        self.st_event=event
        self.mpl_id = self.panel.canvas.mpl_connect(
                                  'motion_notify_event',
    ...
    def dodrag(self, evt):
        ...
        if (mode == 0 or 
            ...
            if not real_owner in owners: return
            ...
            for figobj in owners:
              ...
              mode == 4):
            if target is None: return
            if target.figobj is None: return
            if isinstance(target.figobj, FigImage): mode = 3
            if isinstance(target.figobj, FigContour): mode = 4
            owners = [a() for a in self.cursor_owner]
            real_owner = target.figobj.get_figaxes()
            if not real_owner in owners: return
...
class ifigure_canvas_draghandler(draghandler_base):
    def bind_mpl(self, event): 
        if self.mpl_id is not None: self.unbind_mpl()
        if self.panel.toolbar.mode != '': return
        self.st_event=event
        self.mpl_id = self.panel.canvas.mpl_connect(
                                  'motion_notify_event',
    ...
    def dragstart(self, evt):
        ...
        redraw = False
        for a in self.a:
           if not a.figobj.isDraggable():continue
           if self.d_mode == '':
...
class ifigure_canvas_draghandler_pan(draghandler_base2):
    ...
    def bind_mpl(self, event):        
        if (self.panel.toolbar.mode != '' and
            self.panel.toolbar.mode != 'pan'): return
           ...
           if self.panel.toolbar.pan_all == 0:
               ...
               for figaxes2 in figaxes.get_parent().walk_axes():
                   for a in figaxes2._artists:
                       ...
                       a.start_pan(evt.x+(x1-x0)*w, evt.y+(y1-y0)*h, 1)
               self.pan_mode = 1
        self.dragging = True
        ...
        if self.a is not None:
           ...
           if self.pan_mode == 0:
               ...
               for figaxes2 in figaxes.get_parent().walk_axes():
                   for a in figaxes2._artists:     
                       x1, y1 = figaxes2.getp('area')[:2]               
                       a.drag_pan(1, None, evt.x+(x1-x0)*w, evt.y+(y1-y0)*h)
...
#               w,h=self._canvas().canvas.get_width_height()
               ...
               for figaxes2 in figaxes.get_parent().walk_axes():
                   for a in figaxes2._artists:                               
                       a.end_pan()  
                   requests = canvas.make_range_request_pan(figaxes2, auto=False, 
                                                            requests=requests)
...
class ifigure_canvas_draghandler_zoom(draghandler_base2, 
    ...
    def bind_mpl(self, evt):        
        if (self.panel.toolbar.mode != '' and
            self.panel.toolbar.mode != 'zoom'): return
...
#                        (check[4]+'Contour cursor',         func[4], None),
              ...
              if a is not None: 
                 if a.figobj is not None:
                    ...
                    if fig_axes is not None:
                        menus2.extend(a.figobj.canvas_axes_selection_menu(fig_axes))
        ...
        if len(parent.selection) > 1:
           if parent._check_can_group():
               if len(menus) != 0:
                   menus = menus + [('---', None, None)]
               if all([x().figobj.get_figaxes() is None for x in parent.selection]):
...
#        canvas.draw()           
    ...
    def onGeneratePath(self, evt):
        ...
        ifigure.events.SendPVDrawRequest(book, wait_idle=True, refresh_hl=False)
...
#                          ~wx.TAB_TRAVERSAL)
      ...
      #self.canvas.Unbind(wx.EVT_IDLE) ### testing this to see if it reduces idle time cpu load
...
#                 self.canvas.mpl_connect('resize_event', self.onResize),
   ...
   def set_3dzoom_mode(self, value, rotate_btn=1, pan_btn=2, zoom_btn=3):
       ...
       if f_page is None: return    
       if value:
           for f_axes in f_page.walk_axes():
               if not f_axes.get_3d(): continue
...
#                 hist.stop_record()            
       ...
       if hasattr(evt.guiEvent, 'RawControlDown'):
           if not evt.guiEvent.RawControlDown(): return
       else:
           if not evt.guiEvent.ControlDown(): return           
...
#              print self.toolbar.mode
              if self.toolbar.mode == '':
                  ...
                  if len(self.selection) != 0:
                     ifigure.events.SendSelectionEvent(fig_page, self, self.selection)
                  self.draw_later()
                  dprint1('select  all...')
       evt.guiEvent.Skip()
...
#   def check_pageobj(self, event):
#       hit = False
#       for name, pobj in self._figure.figobj.get_children():
#           if not isinstance(pobj, FigAxes):
#               for a in pobj._artists:
#                   picker = a.get_picker()
#                   hit, extra = picker(a, event)
#                   if hit: 
#                       event.artist=a
#                       break
#           if hit: break 
       ...
       for obj in self._figure.figobj.walk_tree():
           if obj._floating:
               for a in obj.get_artists_for_pick():
                   if isinstance(obj, FigAxes): continue
...
#      hit, extra = cpicker.figure_picker(self._figure, event)
      ...
      for axes in reversed(self._figure.axes):
         if axes is not axes.figobj._artists[0]: continue
            ...
            if len(self.selection) == 0: 
                ...
                if self.axes_selection() is not None:
                    if self.axes_selection().figobj._generic_axes:
                         self._isec = self._figure.figobj.get_iaxes(self.axes_selection().figobj)
                         self.hold_once(True)
            if drag_happend:
...
#                  self.draw_later()
               ...
               if axes is not None:
                   ifigure.events.SendSelectionEvent(axes.figobj, self, self.selection)
                   ...
                   if (self.axes_selection() is None and
                       len(self.selection) == 0):
                       ifigure.events.SendSelectionEvent(self._figure.figobj, self, self.selection)
               if len(self.selection) == 0: self.toolbar.ptype = 'pmode'
            else:
...
#          print 'double click', self.selection
          if self._mpl_artist_click is not None:
               if isinstance(self._mpl_artist_click[0](), matplotlib.text.Text):
...
#      cpicker.activate()
   ...
   def handle_double_click_mpltext(self, event):
       ...
       def finish_text_edit(x, y, txt, target_artist = target_artist, 
                            obj=self, figobj=self._mpl_artist_click[1]().figobj):
           self = obj
   ...
   def handle_double_click_ax(self, event):
       ...
       if ax is not None:
             if hasattr(ax.figobj, 'hndl_dclick'):
                ...
                if self.toolbar.ptype != 'pmode': return
...
#               len(self.selection)> 1):
       ...
       hl = None   
       if hl_range[0] != inf:
           x = [hl_range[0], hl_range[0], hl_range[1], hl_range[1], hl_range[0]]
           y = [hl_range[3], hl_range[2], hl_range[2], hl_range[3], hl_range[3]]   
...
#           item().figobj.write2shell(alist, 'alist')
       if turn_on_gl: self.canvas.disable_alpha_blend()
       self.draw_artist(alist=alist)
       if turn_on_gl: self.canvas.enable_alpha_blend()       
       for item in self.selection:
           if item() is None: continue
           figobj = item().figobj
           if figobj is not None:
...
#          self._draw_request = True
   ...
   def draw_cursor(self):
       ...
       for figobj in self._cursor_owner:
           ...
           for a in cursors:
              if isinstance(a, Line2D): lines.append(a) 
       ...
       for l in lines: self._figure.lines.remove(l)
       if ax is not None:
           for l in collections: ax.collections.remove(l)
           for l in images: ax.images.remove(l)
       ...
       if (self._cursor_mode == 0 or
          ...
          for f_axes in f_page.walk_axes():
              ...
              if geom_util.check_inside(rect, nx, ny):
                  ...
                  f_axes.erase_cursor()
       else: print('Unknown cursor mode')
   ...
   def copy_selection(self, obj = None):
       ...
       if cmap_hint is not None: data["cmap_hint"] = cmap_hint
   ...
   def paste_selection(self, cs=None, return_history=False, return_header=False):
       import ifigure.widgets.dialog as dialog
       f_page = self._figure.figobj
       f_book = f_page.get_parent()
       i_page = f_book.i_page(f_page)

       if self.axes_selection()==None:
           ...
           if header["mode"] == 'page':
              ...
              if f_axes is not None:
                 ...
                 if ret == 'yes':
                    ...
                    # I can not reset artist simply, since I need to use loaded_property
                    ...
                    for name, child in obj.get_children():
                        if isinstance(child, FigAxes):continue
...
#               self.del_selection()
               ...
               if len(self.selection) != 0:
                  for s in self.selection:
                      ...
                      if isinstance(s().figobj, FigAxes): break
                  else:
#                     actions.append(UndoRedoAddRemoveArtists(artists = self.selection, 
#                                         mode = 1))
                      self.del_selection()
...
#       f_axes.realize()
   ...
   def del_selection(self, menu_name='delete'):
       ...
       h =  UndoRedoAddRemoveArtists(artists = self.selection, 
                                     mode = 1)
       window = self.GetTopLevelParent()
       GlobalHistory().get_history(window).make_entry([h], menu_name = menu_name)
       self.selection=[]              
       return 
...
#              self.refresh_hl_idle()
       ...
       if not done:
          ...
          for x in tmp: self.selection.append(x)
...
#      figobj=selections[0]().figobj
      ...
      if mode == 1 or mode == 3:
         flag=True
         for item in self.selection:
             if len(evt.selections) == 0: continue
   ...
   def _set_samexy(self, name, auto=False, mode=1):
       ...
       if (self._figure.figobj.num_axes() == 1 and
           not window._use_samerange): return
       if self.axes_selection is None: return
       if self.axes_selection() is None: return
...
#             matplotlib.rcParams['ps.fonttype'] = 42
       ...
       if wc == 0:
           if ret[-4:] != '.eps': ret = ret + '.eps'
       elif wc == 1:
           if ret[-4:] != '.pdf': ret = ret + '.pdf'
       elif wc == 2:
           if ret[-4:] != '.svg': ret = ret + '.svg'
       elif wc == 3:
           if ret[-4:] != '.png': ret = ret + '.png'
       elif wc == 4:
           if ret[-5:] != '.jpeg': ret = ret + '.jpeg'
       elif wc == 5:
            if ret[-4:] != '.gif': ret = ret + '.gif'
...
#       print self.GetTopLevelParent()
       value = DialogEditList(list6, modal = True, 
               ...
               tip = None, parent=self.GetTopLevelParent())
   ...
   def insert_arrow(self, evt):
       ...
       if (abs(self._insert_st_event.x - evt.x) < 5 and
           abs(self._insert_st_event.y - evt.y) < 5): return
...
#       print 'insert circle', xy, w, h
   ...
   def i_axesselection(self, callback, figaxes):
       ...
       self._iaxesselection_hl  = matplotlib.patches.Rectangle((0, 0), 0.01,
                                              0.01,alpha=0.5, color='red',
   ...
   def iaxesselection_update_redbox(self):
       if self._iaxesselection_figaxes is None: return 
       if self._iaxesselection_hl is None: return 
...
#       print 'update'
   ...
   def line_insert_motion(self, evt): 
       ...
       if evt.guiEvent.ShiftDown():
          dx = self._line_insert_path[-2][1][0]-self._line_insert_path[-1][1][0]
          dy = self._line_insert_path[-2][1][1]-self._line_insert_path[-1][1][1]
          if abs(dx) < abs(dy):
              self._line_insert_path[-1]=(Path.LINETO, 
                          (self._line_insert_path[-2][1][0], evt.y), 
                          0)
          else:
              self._line_insert_path[-1]=(Path.LINETO, 
                          (evt.x, self._line_insert_path[-2][1][1]), 
                          0)
...
#       print 'line insert release', evt.x, evt.y
   ...
   def _check_can_group(self):
       ...
       flag = True
       for s in self.selection:
           if not s().figobj.get_parent() is f: flag = False
       for s in self.selection:
           if isinstance(s().figobj, FigAxes): flag = False
       return flag
