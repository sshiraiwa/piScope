...
class CustomSingleSlider(CustomPanel):
    ...
    def OnMouseEvent(self, evt):
        ...
        if evt.LeftDown():
           x, y = evt.GetPosition()
           w, h = self.GetSize()
           if (abs(y - h/2) >  self._sheight): return 

           w1 = self._calc_w1()
           self._hit = -1
           if abs(x-w1) < self._bwidth: self._hit = 0
           if self._hit > -1:
              self.CaptureMouse()
...
#        if self._hit == 1 and new_v <= self._value[0]: return
        if new_v > self._range[1]: return
        if new_v < self._range[0]: return
...
class CustomDoubleSlider(CustomPanel):
    ...
    def OnMouseEvent(self, evt):
        ...
        if evt.LeftDown():
           ...
           self._hit = -1
           if abs(x-w1) < self._bwidth: self._hit = 0
           if abs(x-w2) < self._bwidth: self._hit = 1
           if self._hit > -1:
              self.Bind(wx.EVT_MOTION, self.OnMotion)
              self.Bind(wx.EVT_MOUSE_CAPTURE_LOST, self.OnMCLost)
              self.CaptureMouse()
#              self.Bind(wx.EVT_LEAVE_WINDOW, self.OnLeave)
    ...
    def OnMotion(self, evt):
        ...
        new_v = float(x - self._hmargin -bmw/2)/dx*delta + self._range[0]
        if self._hit == 0 and new_v >= self._value[1]: return
        if self._hit == 1 and new_v <= self._value[0]: return
        if new_v > self._range[1]: return
        if new_v < self._range[0]: return
        self._value[self._hit] = new_v
