    ...
    def __init__(self, parent, reload = False):
        ...
        if parent.doc_name.endswith('.py'):
            ...
            if parent.check_if_in_script_editor():
                ...
                if sp != ep:
                    menus.append(('Run Selection in Shell (F8)', parent.onRunSelection, None))
...
#                check_debugger_instance()
                ...
                if not has_breakpoint(parent.doc_name, line+1):
                    menus = menus + [('Set BreakPoint',   parent.onSetBP, None),]
                else:
                    menus = menus + [('Clear BreakPoint', parent.onClearBP, None),]
                if parent.is_debug_mode:
                    menus = menus + [('Hide debug setting', parent.HideDebug, None)]
                else:
...
class PythonSTC(stc.StyledTextCtrl):
    ...
    def __init__(self, parent, ID,
        ...
        self.MarkerDefine(DebugBreakPoint,  stc.STC_MARK_CIRCLEMINUS, "red", "white")
...
#        self.Bind(wx.EVT_LEFT_UP, self.onLeftUp)
    ...
    def SaveFile(self, file):
        ...
        if cmtime > self.file_mtime:
           ret=dialog.message(self, 'File is changed after the file was read last time.\nDo you want to overwrite?.','File modified by someone', style=4)           
           print(ret)
           if ret != 'yes':
               ret=dialog.message(self, 'De you want to reload file?.','File modified by someone', style=4)           
               if ret == 'yes':
                  try:
                      fid = open(file)
                      txt=open(file).read()
...
#            return
        ...
        elif key == 75 and controlDown:  #ctrl + K (cut the right of caret, delete line if the line is empty)	
...
#        print self.file_list
    ...
    def onFileSystemChanged(self, evt=None):
        if evt.time <  self._last_update: return
...
#           print os.path.exists(file)
        ...
        if do_layout:  app._force_layout()
...
#        print 'RunSEDRequest', threading.current_thread().name
        if len(self._d_requests) == 0: return
...
#        self.d_panel.CheckDebuggerInstance()
    ...
    def ShowDebugCurrentLine(self, file, line):
        if not file in self.file_list: return
        idx = self.file_list.index(file)
        ipage=self.nb.GetPageIndex(self.page_list[idx])
        p=self.nb.GetPage(ipage)
