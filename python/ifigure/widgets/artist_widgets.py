...
###
listparam["multialignment"] = ["alignment", "left",  4,  s_multialignment, 'multialignment', 2]
...
listparam["markeredgewidth"] = ["edge width",  2.,  7, {}, 'markeredgewidth', 0]
...
listparam["contour_nlevel2"] =  ["levels",  (False, ['7'], ['10']), 27, 
                                  ({"text":'use expression'}, {"elp": mm2}, {"elp":mmm}),
                                  "contour_nlevel2", 2]
mm21 = [['   ',   'red', 106, {}],]
mm2 = [['size', '10',  104,  s_fontsize],
       [None, (False, ['red'])  , 27,({"text":'use fixed color'}, {"elp": mm21})],
...
listparam["spec_fftp"] = [None, ("256", "256"),  33,  {"elp": elp_spec_fftp}, 'spec_fftp', 2]
listparam["spec_noverlap"] = ["Overlap", "128",  104,  s_spec_nfft16, 'spec_noverlap', 2]
listparam["spec_window"] = ["Window", "hanning",  4,  s_spec_window, 'spec_window', 2]
listparam["spec_sided"] = ["Sides", "onesided",  4,  s_spec_sided, 'spec_sides', 2]
listparam["spec_detrend"] = ["Detrend", "none",  4,  s_spec_detrend, 'spec_detrend', 2]

listparam["axrangeparam13"]  = [None,    None,   20, {}, 'axrangeparam13', 13, 'axrangeparam']
...
listparam["axis_bgedgecolor"] = ["edge",   'red', 206, {}, 'axis_bgedgecolor', 2]
...
listparam["page_ticklabel_size"]=["ticklabel_size", 12, 104, {}, 'ticklabel_size', 8]
listparam["page_axeslabel_size"]=["axeslabel_size", 12, 104, {}, 'axeslabel_size', 8]
listparam["page_axesbox_width"]=["axesbox_width", 1, 104, {}, 'axesbox_width', 8]
listparam["page_axestick_width"]=["axestick_width", 1, 104, {}, 'axestick_width', 8]
...
for key in listparam:
   if len(listparam[key]) == 6: 
         if key[-2:] == '_1': name =  key[:-2]
         elif key[-2:] == '_2': name =  key[:-2]
         else: name = key
...
#
       ...
       for k in range(len(self.list)): 
          ...
          for i in range(len(value)):
             ...
             if len(self.tab) > k:
                 value[i] = call_getter(artist, self.list[k][i], self.tab[k])
             else:
   ...
   def set_artist_property(self, evt):
       ...
       if self.target_artist is None: return None, None
       if self.target_artist() is None: return None, None
...
#       print 'enabling contour widget'
       ...
       for elp in self.elp:
           elp.Enable(value)
       if not value: return
       if self.target_artist is None: return
       if self.target_artist() is None: return
...
#       self.panels[mode].Layout()
    ...
    def onTD_Replace(self, evt):
        for mode in self.panels:
           if self.panels[mode].target_artist is None: continue
           if self.panels[mode].target_artist() is None: continue
...
#               ('mdsplus', artist_mdssource_widget),
    ...
    plistd = {}
    for name, cls in plist: plistd[name]=cls
...
class panel2(artist_widgets):
    ...
    def onTD_Selection(self, evt):
        ...
        disable = False
        if axes is None: disable = True
        elif axes() is None: disable = True
        elif not hasattr(axes(), "figobj"): disable = True
        elif axes().figobj is None: disable = True
        ...
        if isinstance(evt.GetTreeDict(), FigPage): return       
