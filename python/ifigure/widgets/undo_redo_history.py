...
#        print self, id(self)
    ...
    def replace_aritst(self, a1,a2):
        pass
    def clear(self):
        if self._proj is not None: del self._proj
        self._proj = None
    def __del__(self):
        if self._proj is not None: del self._proj
        self._proj = None
class Undobase(History):
    ...
    def do2(self):
        return self.figobj
    def update_hl(self):
        if not self.is_hlupdate: return
...
#        print root.get_full_path()
    ...
    def add(self):
        ...
        self.child_idx = []
        if len(o_list) != 0:
            self._ret_path = o_list[0].find_common_parent(o_list).get_full_path()
        else:
            self._ret_path = ''
    ...
    def remove(self):
        ...
        if len(p_list) != 0:            
            self._ret_path = p_list[0].find_common_parent(p_list).get_full_path()                           
        else:
            self._ret_path = ''
        for k in range(len(self.figobj_paths)):
            figobj = self.proj.find_by_full_path(self.figobj_paths[k])
...
#        self.update_menu_item()
    ...
    def rm_history(self, obj):
        self.undostack = [item for item in self.undostack if item.count(obj) == 0]
        self.redostack = [item for item in self.redostack if item.count(obj) == 0]
        self.new_entry = HistoryEntry()
        self.update_menu_item()
...
#        figobj.set_bmp_update(False)        
    ...
    def send_draw_request(self, hist):
        if hist.draw_request == 'draw':
             ...
             ifigure.events.SendCanvasDrawRequest(self.window().canvas, all = True)
