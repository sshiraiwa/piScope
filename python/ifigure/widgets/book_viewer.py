...
#            evt.GetEventObject().Check(True)
            ...
            if len(viewers) == 0:
                ...
                if not app in viewers: viewers.append(app)
                mm = []
                for v in viewers:
                    if v.book is None: continue
...
#        print 'resize idle', self.HasCapture()
    ...
    def call_draw_after_resize(self):
        if  self.canvas._figure is None: return
        if self.book._screen_ratio_lock is not None:
             self.set_canvas_ratio(self.book._screen_ratio_lock)
             self.write_canvas_size_to_status_bar()
        ...
        if id in BookViewerFrame.ID_BOOKNONE_CHECK:
            ...
            if (id == BookViewerFrame.ID_SAVEPROJ):
                ...
                top_hidden = (len(app.viewers) == 1 and    
                              not app.IsShown())
                if top_hidden:
                    evt.Enable(False);evt.Show(False); 
                else:
...
#                     "Copy Page",  "copy current page",  self.onCopyPage)
    ...
    def add_std_plotmenu(self, plotmenu):
        self.add_menu(plotmenu, BookViewerFrame.ID_PM[6], 'Autoscale X', "", self.onXauto)
        self.add_menu(plotmenu, BookViewerFrame.ID_PM[7], 'Autoscale Y', "", self.onYauto)
        ...
        self.add_menu(plotmenu, BookViewerFrame.ID_PM[9], 'Autoscale all Y', "", 
                      self.onYauto_all)
        self.add_menu(plotmenu, BookViewerFrame.ID_PM[10], 'All same X scale', "", self.onSameX)
        self.add_menu(plotmenu, BookViewerFrame.ID_PM[11], 'All same X (Y auto)', "", self.onSameX_autoY)
        self.add_menu(plotmenu, BookViewerFrame.ID_PM[12], 'All same Y scale', "", self.onSameY)
...
#            self._use_samerange_mni.Check(True)
        ...
        hist = [UndoRedoFigobjMethod(p._artists[0], 'viewer_samerange_mode', self._use_samerange_mni.IsChecked())]
    ...
    def onCopyAxes(self, e):
        if self.canvas.axes_selection()==None: return
...
#        self.realize()
        if self.book.num_page() == 0: 
            self.book.set_open(False)
            return
        if not self.book.isOpen: self.book.set_open(True)
...
class BookViewer(BookViewerFrame):
    def __init__(self, *args, **kargs):    
        ...
        if gl: ifigure_canvas.turn_on_gl = gl
...
#        self.Close()
    ...
    def onAttachFigure(self, e):
        ...
        if aviewer is self: 
           w.aviewer = w
    def adjust_attach_menu(self):
        if not self.isattachable: return
        w = wx.GetApp().TopWindow
        if w.book is None:
