...
import wx
import subprocess, wx, os, shlex, socket
...
def run_job(self, a):
    ...
    if self.td._pool is None:
        wx.CallAfter(print_threaderror, ['can not run job. worker pool is not ready'])
        return    
    ...
    while(wait < maxwait):
        ...
        wx.CallAfter(print_threaderror, ['can not run job. waiting time exceeded max time'])
        return    
...
def onStartWorker(self, e=None):
    ...
    if t_type is None: t_type = 'default'
...
def onSetting(self, e=None):
    ...
    list6 = [["Use Proxy", txt, 1, {"values":["on", "off"]}],
           ...
           ["NumWorker", str(ifigure.mdsplus.mdsscope.mds_num_worker), 0, None],
