...
import shutil, os, time, subprocess, traceback
import ifigure
...
def make_header(release = 'piscope_release'):
    source = os.path.dirname(os.path.dirname(os.path.dirname(ifigure.__file__)))
    root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(ifigure.__file__))))
...
def make_release(release = 'piscope_release', version = 'beta'):
    ...
    #            print 'processed ', path
        ...
        fid.write('ifig_version = "'+ time.strftime('%Y %B %d')+'_'+ version   + ' with OpenGL backend"\n')
        fid.close()
