...
def  on_off_args(v):
     ans = False
     if isinstance(v, str):
        if v.upper()=='ON': ans = True
        if v.upper()=='OFF': ans = True
        if v.upper()=='1': ans = True
        if v.upper()=='0': ans = False
...
def FindFrame(w):
    if isinstance(w, wx.Frame): return w
    w = w.GetParent()
    if w is not None: return FindFrame(w)
...
def GetArtistExtent(a, box=None, renderer=None, canvas=None, force=False):
    ...
    if hasattr(a, 'artists'):
        for a in a.artists():
           box = GetArtistExtent(a, box=box, renderer=renderer, canvas=canvas, force=True)
...
def parseStr(x0):
   ...
   if x.startswith('-'): 
      ...
      sign=-1
   try:
      if x1.isdigit(): return [int(x1)*sign]*num
   except:
      pass   
