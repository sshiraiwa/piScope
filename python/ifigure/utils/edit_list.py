    ...
    def onHit(self, evt):
        ...
        if str(self.cb.GetValue()) == 'customize...':
            if self.value0 is None:
                ...
                is_ok, value = ask_setting(self, self._value0, self.choices[:2])
            if not is_ok: return
...
class AxisRange(wx.Panel):
    ...
    def _check_order(self):
         if not self.check_range_order: return
...
#          print 'expanding'
    ...
    def add_bitmap_buttons(self, title, ftitle, names, pname, 
        ...
        if filenames is None: filenames = names
        self._btn = [None]*len(names)
        self._btn_name = names
        i = 0
        if labels is None: labels = [None]*len(names)
...
class Color(BitmapButtons):
    ...
    def __init__(self, *args, **kargs):
        ...
        **kargs is not used for now
        ...
        self.add_bitmap_buttons('Color', 'color', names , 'color', filenames=fnames)
        ...
        for ctl in self.Controls:
            if ctl["obj"] is evt.GetEventObject():
               if ctl["value"] == 'other':
                   ...
                   v =  data.GetColour() 
                   color = v.alpha*256*256*256 + v[2]*256*256 + v[1]*256 + v[0]
                   dlg.Destroy()        
                   self.val = color
               else:
                   self.val = ctl["value"]
...
class ColorSelector(wx.BitmapButton):
    def __init__(self, *args, **kargs):
        ...
        wx.BitmapButton.__init__(self, *args, bitmap=wx.Bitmap(self.imageFiles['blue']))
        self.value = 'blue'
    ...
    def onHit(self, evt):
        ...
        dia.Destroy()
        if val != wx.ID_OK: return
        if value[0]: self.SetValue(value[1])
        evt.Skip()
        self.GetParent().send_event(self, evt)
...
class TextCtrlCopyPasteGenericHistory(TextCtrlCopyPasteGeneric):
    ...
    def onKeyPressed(self, event):
        ...
        if key == wx.WXK_UP:
            if len(self._key_history_st1) == 0: return
            ...
            if len(self._key_history_st2) == 0: return
...
class Slider(wx.Panel):
    ...
    def _Update(self, evt):
        if evt.GetEventObject()==self.s1:
           val=self.GetValue()
           if self.t1 is not None: self.t1.SetValue(str(val))
...
class CAxisSelector(ComboBoxCompact):
    ...
    def SetValue(self, value):
        if self.GetTopLevelParent().canvas is None: return
        value = str(value)
    ...
    def Enable(self, value):
        if value: self._check_update_selection()
    ...
    def _check_update_selection(self):
        ...
        if self.GetTopLevelParent().canvas is None: return
        if len(self.GetTopLevelParent().canvas.selection) == 0: return
        if self.GetTopLevelParent().canvas.selection[0]() is None: return
        if not isinstance(self.GetTopLevelParent().canvas.selection[0]().figobj, CUser): return

        choices = self.GetTopLevelParent().canvas.selection[0]().figobj.get_caxis_choices()
    ...
    def _update_choices(self, c):
        ...
        self._choices = c
        if v in self._choices: super(CAxisSelector, self).SetValue(v)
...
#       self.GetSizer().Add(self.elp,  1, wx.EXPAND)
    ...
    def GetValue(self):
        ...
        for i in range(len(self.panels[self.mode])):
            val.append(self.panels[self.mode][i]+'='+str(self.elp.GetValue()[i]))
        s = ','.join(val)
    ...
    def SetValue(self, value):
        ...
        if self.elp is not None:
           ...
           if len(value)!=0: self.elp.SetValue(value)
...
#        self.SetValue('-')
    ...
    def GetValue(self):
        ...
        for name, style in self.choice_list:
            if val == name: return style
            if val == style: return style
        return 'simple'
...
#        print 'onSize', self.GetSize(), self.GetParent().GetSize() 
    ...
    def onPageClose(self, evt):
        ...
        if str(label) in ['x', 'y', 'z', 'xerr', 'yerr']:
            ...
            evt.Veto()
            return
        if label in self._always: self._always.remove(label)
    ...
    def SetValue(self, value):
        ...
        for key in sigs:
            if value[key] is None:  sigs.remove(key)
        
        sigs2 = []
        for name in sigs:
            if not name in MDSSource0.tag_order: sigs2.append(name)
...
#        self.elp.SetValue(value["data"])
        ...
        sessions = [child for name, child in ax.get_children() if isinstance(child, FigMds)]
        labels = []
        for s in sessions:
            v = s.getvar('mdsvars')
...
#                 setting={"reverse": False}
              ...
              choices = ['c']
              if len(val)==4 and val[3] is not None:
                 if 'choices' in val[3]: choices = val[3]['choices']
                 if 'style' in val[3]: style = val[3]['style']

              w = CAxisSelector(self, wx.ID_ANY, style=style, choices = choices)
              if val[1] is not None: w.SetValue(val[1])
#              w.SetValue(val[1])
              p = w
           elif val[2] == 29:              
...
#              self.Bind(wx.EVT_TEXT_ENTER, self._textctrl_enter, w)
        ...
        if len(list) > 0: sizer.AddGrowableCol(1)
        self.SetSizer(sizer)
        self.list = list
...
def _DialogEditListCore(list, modal = True, style = wx.DEFAULT_DIALOG_STYLE,
    ...
    if modal:
       ...
       def ok_func(evt):
           ...
           value = diag.GetValue()
           if not ok_noclose:  diag.Destroy()
           ok_cb(value)
